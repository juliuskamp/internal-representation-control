...
VENDORED from https://github.com/kitft/nla-inference
  commit 38b802a33d1d317f21b6825a9116f388c2141f86, Apache-2.0 (LICENSE in this
  directory). Local modification: `return_dict=False` added to both
  `apply_chat_template` calls — transformers >=4.57 returns a BatchEncoding by
  default, whose iteration yields dict keys and breaks the injection-position
  scan. No other changes; diff against upstream to sync.
...
def load_nla_config(
    ...
    # return_dict=False: transformers >=4.57 returns a BatchEncoding by
    # default, whose iteration yields keys, breaking the position scan below.
